...
def create_cnn():
    class cnn(nn.Module):
        def __init__(self):
            ...
            self.layer_fc1 = nn.Linear(85536, fc_size)
...
def plot_images(images, cls_true, cls_pred=None, iterations=0):
    ...
    # plt.show()
    plt.savefig("plot_images" + str(iterations)  + "iter " + str(time.time()) + ".jpg")


def plot_confusion_matrix(cls_pred, cls_true, iterations):
    ...
    # in a single Notebook cell.
    # plt.show()
    plt.savefig("plot_confusion_matrix" + str(iterations)  + "iter " + str(time.time()) + ".jpg")



def test(dataloader, model, loss_fn, show_example_errors=False, show_confusion_matrix=False, iterations=0):
    size = len(dataloader.dataset)
    ...
    if show_example_errors:
        ...
        plot_images(images=mis_example[:9], cls_true=mis_example_true[:9], cls_pred=mis_example_pred[:9], iterations=iterations)

    # Plot the confusion matrix, if desired.
    if show_confusion_matrix:
        print("Confusion Matrix:")
        plot_confusion_matrix(cls_pred=y_pred, cls_true=y_true, iterations=iterations)
...
device = None
def run(lm=1e-4, no_of_hidden_layers=50):
    learning_rate = lm
    train_batch_size = 50
    test_batch_size = 100
    ...
    # load data
    trainset = datasets.CelebA(root='./data', split='train', download=False, transform=transforms.ToTensor(), target_type='attr')
    testset = datasets.CelebA(root='./data', split='test',  download=False, transform=transforms.ToTensor(), target_type='attr')
    ...
    test(test_dataloader, model, cost, True, True, 1)

    # We already performed 1 iteration above.
    optimize(9, train_dataloader, model, cost, optimizer)
    test(test_dataloader, model, cost, True, True, 9)

    optimize(99, train_dataloader, model, cost, optimizer)
    test(test_dataloader, model, cost, True, True, 99)

    optimize(999, train_dataloader, model, cost, optimizer)
    test(test_dataloader, model, cost, True, True, 999)

    optimize(9000, train_dataloader, model, cost, optimizer)
    test(test_dataloader, model, cost, True, True, 9000)
